import { Component, OnInit ,ChangeDetectorRef, Output, EventEmitter, Input, ViewChildren, QueryList,SimpleChanges} from '@angular/core';
...
export class ResFilterComponent implements OnInit {
  ...
  @Input() filtermobiletop:string='FilterRec';
  ...
  ngOnInit() {
    ...
    if (this.srv_appsetting.isMobileResolution) 
    {
      ...
      this.filterBy = this.filtermobiletop;
    }
  ...
  ApplyMobile()
  {
    //this.parentComponent.ChangeFilterMobile();
  ...
  ngOnChanges(changes:SimpleChanges) {
    for (let property in changes) {
      if (property === 'filtermobiletop') {                
        this.filtermobiletop= changes['filtermobiletop'].currentValue;  
        this.filterBy=   this.filtermobiletop; 
       } 
      }
    }
